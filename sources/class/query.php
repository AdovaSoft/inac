...
// Find every $col from $table
     ...
     * @return int
     */
    public function update_column($table, $cols, $vals, $flag, $mat, $con, $val, $extra = null)
    {
        $query = $this->gen_update_query($table, $cols, $vals, $flag, $mat, $con, $val, $extra);
        if (mysqli_query($this->dtb_con, $query)) {
            return 1;
        } else {
            return 0;
     ...
     * @return string
     */
    public function gen_update_query($table, $cols, $vals, $flag, $mat, $con, $m_val, $extra = null)
    {
        $qur = sprintf("UPDATE %s SET", $table);
        $set = '';
        for ($i = 0; $i < count($cols); $i++) {
            if ($flag[$i] == "s") {
                $set .= sprintf(" %s = '%s'", $cols[$i], $vals[$i]);
            } else {
                $set .= sprintf(" %s = %s", $cols[$i], $vals[$i]);
            }
        }
        ...
        $qur .= $set;
        if ($extra != null)
            return "$qur WHERE $extra";

        else
            return "$qur WHERE $mat $con $m_val";
    }
