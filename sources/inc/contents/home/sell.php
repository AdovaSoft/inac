    ...
    if (strlen($s) < 1) {
        ...
        if (count($sell_results) > 0) {
            ...
            for ($i = 0; $i < count($sell_results); $i++) {
                ...
                echo "<form method='POST' style='margin: 0px'><input type='hidden' name='searchword' value='" . $_POST['searchword'] . "'/>
                <input type='hidden' name='sell_id' value='" . $sell_results[$i][0] . "'/>
                <input type='submit' name='delete_sell' value='Delete'/></form> ";
                echo "<form method='POST'  style='margin: 0px' action='index.php?e=" . $encptid . "&page=sells&sub=return'>
                <input type='hidden' name='v' value='" . $sell_results[$i][0] . "'/>
                <input type='submit' name='ab' value='Edit'/></form> ";
                echo "<form method='POST'  style='margin: 0px' id='printBox' action='print.php?e=" . $encptid . "&page=sells&sub=sell' target='_blank'>
